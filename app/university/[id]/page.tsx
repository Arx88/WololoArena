        ...
        <Card className="bg-[#0a0a0b]/95 border-white/5 backdrop-blur-3xl rounded-[4rem] shadow-[0_0_150px_rgba(0,0,0,1)] overflow-hidden">
          <CardContent className="p-16 text-center flex flex-col items-center">
            ...
            <div className="flex items-center gap-4 mb-8">
               ...
               <Badge variant="outline" className="border-white/10 text-white/40 px-8 py-1.5 uppercase tracking-[0.5em] text-[10px] font-black font-mono">Final Report</Badge>
               <div className="h-px w-8 bg-white/10" />
            </div>
            
            <h2 className="text-6xl md:text-7xl font-black italic uppercase tracking-tighter text-white mb-4 leading-none font-cinzel">
            ...
            <div className="grid grid-cols-3 gap-6 w-full mb-16">
               <div className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 shadow-inner">
                  <span className="block text-[10px] font-black text-white/20 uppercase mb-3 tracking-widest font-mono">Accuracy</span>
                  <span className="text-4xl font-black italic text-white leading-none font-cinzel">{percentage}%</span>
               </div>
               <div className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 shadow-inner">
                  <span className="block text-[10px] font-black text-white/20 uppercase mb-3 tracking-widest font-mono">Score</span>
                  <span className="text-4xl font-black italic text-primary leading-none font-cinzel">{score}<span className="text-white/20 text-xl">/{total}</span></span>
               </div>
               <div className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 shadow-inner">
                  <span className="block text-[10px] font-black text-white/20 uppercase mb-3 tracking-widest font-mono">Status</span>
                  <span className={cn("text-xl font-black italic uppercase leading-none font-cinzel", isPassed ? "text-emerald-500" : "text-red-500")}>{isPassed ? "Certified" : "Denied"}</span>
               </div>
            </div>
            ...
            <div className="flex gap-6 w-full">
               ...
               {!isPassed ? (
                 ...
                 <Button asChild className="flex-1 h-20 rounded-none bg-yellow-600 text-black font-black uppercase tracking-[0.3em] hover:bg-yellow-500 transition-all border-r-8 border-black/20 shadow-[0_0_50px_rgba(234,179,8,0.3)]">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="relative p-12 rounded-[3rem] bg-[#0a0a0b]/80 border border-white/5 backdrop-blur-xl shadow-2xl overflow-hidden mb-10">
                <div className="absolute top-0 right-0 p-12 text-[160px] font-black text-white/[0.01] italic select-none pointer-events-none uppercase leading-none">
                ...
                <h2 className="text-4xl md:text-5xl font-black text-white uppercase italic tracking-tighter leading-[0.9] mb-16 relative z-10 max-w-3xl font-cinzel">
              ...
              <AnimatePresence>
                {isAnswered && (
                  <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} className="relative">
                    <div className={cn(
                      "p-1 rounded-[2.5rem] bg-gradient-to-r shadow-2xl",
                      selectedOption === currentQuestion.correctAnswer ? "from-emerald-500/40 via-transparent to-transparent" : "from-red-500/40 via-transparent to-transparent"
                    )}>
                      <div className="p-10 rounded-[2.4rem] bg-[#050506] flex items-center justify-between gap-10">
                        <div className="flex items-center gap-8">
                          <div className={cn(
                            "h-20 w-20 rounded-3xl flex items-center justify-center shrink-0 shadow-2xl",
                          ...
                          <div className="space-y-2">
                            <h4 className="text-[11px] font-black uppercase text-primary tracking-[0.4em] flex items-center gap-2">Analysis <span className="h-1 w-8 bg-primary/20 rounded-full" /></h4>
                            <p className="text-xl text-white/80 italic font-medium leading-relaxed max-w-2xl font-cinzel">{currentQuestion.explanation}</p>
                          </div>
                        </div>
                        <Button onClick={handleNext} className="h-20 px-12 rounded-none bg-primary text-black font-black uppercase text-sm tracking-[0.3em] hover:bg-white transition-all border-r-8 border-black/20 hover:scale-105 shadow-2xl">
                          Protocol Next <ChevronRight className="ml-4 h-6 w-6" />
                        </Button>
                      </div>
                    </div>
                  </motion.div>
