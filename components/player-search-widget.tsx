...
import { Search, Globe, Activity, X, Loader2, Sword, Swords, Shield, Target, Crown, ChevronDown } from "lucide-react"
...
import { VersusHistoryWidget } from "./versus-history-widget"
...
export function PlayerSearchWidget() {
  ...
  const [showVersus, setShowVersus] = React.useState(false)
  ...
  return (
    <>
      ...
      <AnimatePresence mode="wait">
        {!isOpen && !showVersus && (
            <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] flex items-stretch gap-2">
                ...
                <motion.button
                    ...
                    className="flex items-center gap-4 px-10 py-5 bg-black border-2 border-yellow-600/30 text-yellow-500 font-black uppercase tracking-[0.3em] text-xs transition-all shadow-[0_0_50px_-10px_rgba(234,179,8,0.4)] border-r-8 border-yellow-600 group rounded-none"
                >
                ...
                <motion.button
                    ...
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    onClick={(e) => { e.stopPropagation(); setShowVersus(true); }}
                    className="flex items-center justify-center w-16 bg-black border-2 border-yellow-600/30 text-yellow-500 transition-all shadow-[0_0_50px_-10px_rgba(234,179,8,0.2)] border-l-4 border-yellow-600 group"
                >
                    <div className="relative">
                        <Swords className="w-6 h-6 transition-transform group-hover:rotate-12" />
      ...
      {/* VERSUS HISTORY MODAL OVERLAY - MOVED TO TOP LEVEL */}
      <AnimatePresence>
        {showVersus && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-8">
                ...
                <motion.button
                    ...
                    className="absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 group"
                >
                    <span className="text-[8px] font-black text-white/20 uppercase tracking-[0.3em] group-hover:text-yellow-500/50 transition-colors">Dismiss Versus</span>
                    <div className="h-10 w-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center backdrop-blur-md group-hover:bg-yellow-500/20 group-hover:border-yellow-500/30 transition-all">
                        <ChevronDown className="w-5 h-5 text-white/40 group-hover:text-yellow-500 group-hover:translate-y-0.5 transition-all" />
                    </div>
                </motion.button>
